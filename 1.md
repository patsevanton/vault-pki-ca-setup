...
Экспортируйте значения по умолчанию чарта Vault в файл values.yaml:
```shell
...
server:
  ...
  ha:
    enabled: true
    # Количество реплик Vault. Для кворума Raft рекомендуется нечетное число (3 или 5).
    replicas: 3
    # Включаем встроенное хранилище Raft.
    raft:
      enabled: true
      # Настройки для постоянного хранения данных Raft
      config: |
        ui = true
        ...
        # Листенер на порту 8200 для HTTP (будет изменен позже на HTTPS)
        listener "tcp" {
          address = "[::]:8200"
        ...
        # API адреса для взаимодействия узлов кластера между собой
        api_addr = "http://${POD_NAME}.vault-internal:8200"
        cluster_addr = "https://_POD_IP_:8201"
  
  # Отключаем режим разработки (dev mode)
  dev:
    enabled: false
```
