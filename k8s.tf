...
resource "yandex_kubernetes_cluster" "vault" {
  name       = "vault"  # Имя кластера
...
resource "yandex_kubernetes_node_group" "k8s-node-group" {
  ...
  name        = "k8s-node-group"
  cluster_id  = yandex_kubernetes_cluster.vault.id
  ...
  allocation_policy {
    ...
    location { zone = yandex_vpc_subnet.vault-a.zone }
    location { zone = yandex_vpc_subnet.vault-b.zone }
    location { zone = yandex_vpc_subnet.vault-d.zone }
  }
  ...
  instance_template {
    ...
    network_interface {
      ...
      subnet_ids = [
        yandex_vpc_subnet.vault-a.id,
        yandex_vpc_subnet.vault-b.id,
        yandex_vpc_subnet.vault-d.id
      ]
    }
...
provider "helm" {
  kubernetes {
    host                   = yandex_kubernetes_cluster.vault.master[0].external_v4_endpoint  # Адрес API Kubernetes
    cluster_ca_certificate = yandex_kubernetes_cluster.vault.master[0].cluster_ca_certificate  # CA-сертификат
